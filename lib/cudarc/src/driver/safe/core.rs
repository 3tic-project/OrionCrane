...
pub struct CudaContext {
    ...
    pub(crate) graph_capture_retention_depth: AtomicUsize,
    pub(crate) graph_capture_retained_allocations: Mutex<Vec<RetainedCudaAllocation>>,
    pub(crate) graph_capture_retained_host_allocations: Mutex<Vec<Vec<u8>>>,
...
impl CudaContext {
    ...
    pub fn new(ordinal: usize) -> Result<Arc<Self>, DriverError> {
        ...
        let ctx = Arc::new(CudaContext {
            ...
            graph_capture_retention_depth: AtomicUsize::new(0),
            graph_capture_retained_allocations: Mutex::new(Vec::new()),
            graph_capture_retained_host_allocations: Mutex::new(Vec::new()),
...
#[derive(Debug)]
pub(crate) struct RetainedCudaAllocation {
    cu_device_ptr: sys::CUdeviceptr,
    free_async: bool,
    stream: Arc<CudaStream>,
}

impl RetainedCudaAllocation {
    pub(crate) fn new(
        cu_device_ptr: sys::CUdeviceptr,
        free_async: bool,
        stream: Arc<CudaStream>,
...
impl Drop for RetainedCudaAllocation {
    fn drop(&mut self) {
        let ctx = &self.stream.ctx;
        let cu_device_ptr = std::mem::replace(&mut self.cu_device_ptr, 0);
        if cu_device_ptr == 0 {
            return;
        }
        if self.free_async {
            ctx.record_err(unsafe { result::free_async(cu_device_ptr, self.stream.cu_stream) });
        } else {
            ctx.record_err(self.stream.synchronize());
            ctx.record_err(unsafe { result::free_sync(cu_device_ptr) });
        }
    }
}

impl CudaContext {
    pub(crate) fn begin_graph_allocation_retention(&self) {
        let previous = self
            .graph_capture_retention_depth
            .fetch_add(1, Ordering::Relaxed);
        if previous == 0 {
            self.graph_capture_retained_allocations
                .lock()
                .expect("graph capture retained allocations mutex poisoned")
                .clear();
            self.graph_capture_retained_host_allocations
                .lock()
                .expect("graph capture retained host allocations mutex poisoned")
                .clear();
        }
    }

    pub(crate) fn end_graph_allocation_retention(
        &self,
    ) -> (Vec<RetainedCudaAllocation>, Vec<Vec<u8>>) {
        let previous = self
            .graph_capture_retention_depth
            .fetch_sub(1, Ordering::Relaxed);
        if previous <= 1 {
            self.graph_capture_retention_depth
                .store(0, Ordering::Relaxed);
            let allocations = std::mem::take(
                &mut *self
                    .graph_capture_retained_allocations
                    .lock()
                    .expect("graph capture retained allocations mutex poisoned"),
            );
            let host_allocations = std::mem::take(
                &mut *self
                    .graph_capture_retained_host_allocations
                    .lock()
                    .expect("graph capture retained host allocations mutex poisoned"),
            );
    ...
    pub(crate) fn is_graph_allocation_retention_active(&self) -> bool {
        self.graph_capture_retention_depth.load(Ordering::Relaxed) > 0
    }

    pub(crate) fn retain_graph_allocation(&self, allocation: RetainedCudaAllocation) {
        self.graph_capture_retained_allocations
            .lock()
            .expect("graph capture retained allocations mutex poisoned")
            .push(allocation);
    ...
    pub(crate) fn retain_graph_host_slice<T>(&self, src: &[T]) -> (*const u8, usize) {
        ...
        let retained = retained
            .last()
            .expect("retained host allocation must exist after push");
        (retained.as_ptr(), retained.len())
    }
}
...
impl<T> Drop for CudaSlice<T> {
    fn drop(&mut self) {
        let ctx = &self.stream.ctx;
        if ctx.is_graph_allocation_retention_active() {
            let cu_device_ptr = std::mem::replace(&mut self.cu_device_ptr, 0);
            if cu_device_ptr != 0 {
                ctx.retain_graph_allocation(RetainedCudaAllocation::new(
                    cu_device_ptr,
                    self.free_async,
                    self.stream.clone(),
                ));
...
impl CudaStream {
    ...
    pub fn null<T>(self: &Arc<Self>) -> Result<CudaSlice<T>, result::DriverError> {
        self.ctx.bind_to_thread()?;
        let free_async = self.ctx.has_async_alloc && !self.ctx.is_graph_allocation_retention_active();
        let cu_device_ptr = if free_async {
    ...
    pub fn memcpy_htod<T: DeviceRepr, Src: HostSlice<T> + ?Sized, Dst: DevicePtrMut<T>>(
        ...
        if self.ctx.is_graph_allocation_retention_active() {
            let (src, byte_len) = self.ctx.retain_graph_host_slice(src);
            let src = unsafe { std::slice::from_raw_parts(src, byte_len) };
...
impl CudaStream {
    ...
    pub unsafe fn upgrade_device_ptr<T>(
        ...
        CudaSlice {
            cu_device_ptr,
            free_async: self.ctx.has_async_alloc,
            len,
            read,
            write,
            stream: self.clone(),
