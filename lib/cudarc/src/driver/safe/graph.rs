...
use super::{core::RetainedCudaAllocation, CudaStream, DriverError};
...
pub struct CudaGraph {
    ...
    _retained_allocations: Vec<RetainedCudaAllocation>,
    _retained_host_allocations: Vec<Vec<u8>>,
}
...
impl CudaStream {
    ...
    pub fn begin_capture(&self, mode: sys::CUstreamCaptureMode) -> Result<(), DriverError> {
        ...
        self.ctx.begin_graph_allocation_retention();
        Ok(())
    }
    ...
    pub fn end_capture(
        ...
        self.end_capture_with_flags(flags as u32 as u64)
    }

    /// See [cuda docs](https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__STREAM.html#group__CUDA__STREAM_1g03dab8b2ba76b00718955177a929970c)
    ///
    /// `flags` is passed to [cuGraphInstantiate](https://docs.nvidia.com/cuda/cuda-driver-api/group__CUDA__GRAPH.html#group__CUDA__GRAPH_1gb53b435e178cccfa37ac87285d2c3fa1).
    /// This variant accepts the raw bitmask so callers can use zero flags.
    pub fn end_capture_with_flags(
        self: &Arc<Self>,
        flags: u64,
    ) -> Result<Option<CudaGraph>, DriverError> {
        self.ctx.bind_to_thread()?;
        let end_capture = unsafe { result::stream::end_capture(self.cu_stream) };
        let (retained_allocations, retained_host_allocations) =
            self.ctx.end_graph_allocation_retention();
        let cu_graph = match end_capture {
            Ok(cu_graph) => cu_graph,
            Err(error) => return Err(error),
        };
        if cu_graph.is_null() {
            return Ok(None);
        }
        let cu_graph_exec = match unsafe { result::graph::instantiate_with_flags(cu_graph, flags) } {
            Ok(cu_graph_exec) => cu_graph_exec,
            Err(error) => {
                self.ctx
        ...
        Ok(Some(CudaGraph {
            ...
            cu_graph_exec,
            stream: self.clone(),
            _retained_allocations: retained_allocations,
            _retained_host_allocations: retained_host_allocations,
        }))
    }
